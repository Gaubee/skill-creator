/**
 * ChromaDB-based search engine with default embeddings
 */

import type { DefaultEmbeddingFunction } from '@chroma-core/default-embed'
...
export class ChromaSearchEngine {
  private db?: ChromaClient
  ...
  async initialize(): Promise<void> {
    ...
    // For now, always use HTTP client with local ChromaDB server
    // The JavaScript client doesn't properly support embedded mode
    console.log('Connecting to ChromaDB server at http://localhost:8000')
  ...
  async search(
    ...
    return results.ids[0].map((id: any, index: number) => ({
      id,
      title: (results.metadatas[0]?.[index]?.title as string) || 'Untitled',
      content: (results.documents[0]?.[index] as string) || '',
      score: (results.distances[0]?.[index] as number) || 0,
      source: (results.metadatas[0]?.[index]?.source as 'user' | 'context7') || 'unknown',
      file_path: (results.metadatas[0]?.[index]?.file_path as string) || '',
  ...
  async searchByPriority(query: string, topK: number = 5): Promise<SearchResult[]> {
    ...
    const contextResults = await this.search(query, topK - userResults.length, {
      source: 'context7',
    })
